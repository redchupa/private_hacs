...
토큰이 만료되거나 재발급한 경우:
1. 설치된 에드온의 **설정값을 백업** (스크린샷 또는 메모)
2. **Repositories** 에서 기존 URL 삭제 → ⚠️ 이때 **설치된 에드온도 함께 제거**됩니다
3. 새 토큰 URL로 다시 추가
4. 에드온 **재설치** 후 설정 복구

> ⚠️ Supervisor는 저장소 URL을 삭제하면 그 저장소에서 설치한 에드온을 함께 제거합니다. (Private HACS의 통합은 등록 해제해도 설치 정보가 보존되지만, 에드온은 다르게 동작합니다.)

### 4단계: 저장소를 다시 Public으로 전환할 때

토큰 없이 사용하고 싶어 GitHub에서 저장소를 public으로 되돌린 경우, 다음 두 가지 방식 중 선택할 수 있습니다.

**옵션 A — URL 그대로 두기 (재설치 불필요, 간편)**
- GitHub에서 저장소만 public으로 전환하면, 기존 토큰 URL은 그대로 작동합니다.
- 단, HA 백업 파일과 Supervisor 로그에 토큰이 그대로 남습니다.

**옵션 B — URL 교체 (재설치 필요, 보안상 권장)**
1. 설치된 에드온의 **설정값을 백업**
2. **Repositories** 에서 기존 토큰 URL 삭제 → 에드온도 함께 제거됨
3. 토큰 없는 일반 URL로 재등록:
   ```
   https://github.com/<user>/<repo>
   ```
4. 에드온 **재설치** 후 설정 복구
5. 사용하지 않는 PAT는 https://github.com/settings/tokens 에서 **Revoke**
